...
숲(SOOP) 라이브 방송과 VOD를 라디오처럼 들을 수 있는 크롬 확장 프로그램입니다.
...
- SOOP 라이브나 VOD를 켭니다.
- `Alt + R` 을 누르면 화면은 꺼지고 소리만 남습니다.
- 인터넷 사용량을 줄이면서, 방송을 라디오처럼 들을 수 있습니다.
- 보고 싶어지면 `비디오 모드로 전환` 버튼을 눌러 바로 원래 화면으로 돌아갑니다.

```mermaid
flowchart LR
  A[SOOP 라이브 / VOD 열기] --> B[Alt + R 누르기]
  B --> C[라디오 모드 진입]
  C --> D[영상 OFF + 오디오 유지]
  C --> E[저화질로 자동 전환]
  C --> F[채팅 / EQ / 볼륨 조절]
...
- 일하거나 공부하면서 방송은 소리만 듣고 싶을 때
- 데이터나 배터리를 조금이라도 아끼고 싶을 때
- 다시보기(VOD)를 오디오북처럼 넘겨 들을 때
- 갑자기 누가 옆에 와서 화면을 빨리 숨겨야 할 때
...
다시보기 페이지에서는 큰 영상 카드 대신 **현재 재생시간 / 총 길이 / 재생바** 중심으로 바뀝니다.  
재생바에 마우스를 올리면 해당 구간 썸네일이 뜨고, 클릭하면 바로 이동할 수 있습니다.

| 보고 싶을 때 | 듣고 싶을 때 |
|:---|:---|
| 일반 비디오 모드 | VOD 라디오보드 |
| 영상 + 소리 | 소리 중심 + 재생바 |
| 화면을 계속 봐야 함 | 필요한 구간만 썸네일로 확인 |
